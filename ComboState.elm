...
addPath (xs, act) tree = case xs of
  [] -> case tree of
    ...
    Leaf _     -> Leaf act
  k::ks -> case tree of 
    Leaf act'    -> Node <| extend (singleton Expire (Leaf act')) (Event k) (addPath (ks, act) <| emptyTree)
    Node delta -> case eval delta (Event k) of
      Nothing -> Node <| extend delta (Event k) (addPath (ks, act) <| emptyTree)
...
{--
main : Signal Element
main = (\x -> flow down (List.map show x)) <~ (Signal.foldp (::) [] withExpires)
--}
...
respondTo : List (Combo, a) -> Signal a
--}
